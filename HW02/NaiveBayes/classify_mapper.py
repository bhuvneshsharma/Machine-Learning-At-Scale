...
for line in sys.stdin:
    ...
    pHam = MODEL.get('ClassPriors', (0,0))[0] + sum(MODEL.get(word,(0,0))[0] for word in words)
    pSpam = MODEL.get('ClassPriors', (0,0))[1] + sum(MODEL.get(word,(0,0))[1] for word in words)
    pHam = np.exp(pHam)
    pSpam = np.exp(pSpam)
    if pHam >= pSpam:
        pred_class = 0
    ...
    print "{}\t{}\t{:.10f}\t{:.10f}\t{}".format(docID, _class, pHam, pSpam, pred_class)